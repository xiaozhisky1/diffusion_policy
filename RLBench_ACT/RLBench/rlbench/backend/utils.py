...
def task_file_to_task_class(task_file):
  # 导入 importlib 模块，它提供了动态导入和重新加载模块的功能
  import importlib
  # 将任务文件名的扩展名 ".py" 去掉，得到任务名（例如 "pick_and_place.py" -> "pick_and_place"）
  name = task_file.replace('.py', '')
  # 将任务名中的每个单词的首字母大写，并去掉下划线，得到类名（例如 "pick_and_place" -> "PickAndPlace"）
  class_name = ''.join([w[0].upper() + w[1:] for w in name.split('_')])
  # 动态导入 rlbench.tasks 模块下的任务文件对应的模块（例如 "pick_and_place" -> "rlbench.tasks.pick_and_place"）
  mod = importlib.import_module("rlbench.tasks.%s" % name)
  # 重新加载该模块，以确保模块的最新版本被加载（有时模块可能在其他地方发生了更改）
  mod = importlib.reload(mod)
  # 获取模块中与任务文件名对应的类（即类名是根据文件名生成的类名）
  # 例如对于任务文件名 "pick_and_place.py"，我们会获取 "PickAndPlace" 类
